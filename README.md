...
Обратите внимание на то, что при подключении свг-спрайта используется `#icon-` префикс в пути до спрайта: `#icon-some-vector-image` (фактически будет использован `some-vector-image.svg`).
